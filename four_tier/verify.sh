...
###############################################################################
# VERIFY EXTERNAL CA CERTIFICATES
###############################################################################
heading_msg "ROOT CA"

title_msg "Check it is a CA certificate..."
result=$(openssl x509 -noout -text -in $ext_intermediate_chain | grep 'CA:TRUE')
if [ "$?" -eq 0 ]; then
    success_msg "Root CA is a CA certificate"
else
    fail_msg "Error: the certificate is not a CA certificate (missing 'CA:True')"
fi

result=$(openssl x509 -noout -text -in $ext_intermediate_pem | grep 'CA:TRUE')
if [ "$?" -eq 0 ]; then
    success_msg "Intermediate CA is CA certificate"
else
    fail_msg "Error: the external Intermediate certificate is not a CA certificate (missing 'CA:True')"
fi

result=$(openssl verify -CAfile $ext_intermediate_chain $ext_intermediate_pem)
if [ "$?" -eq 0 ]; then
    success_msg "External Intermediate CA is signed by the External Root CA"
else
    fail_msg "Error: the external Intermediate certificate is not signed by the external Root CA"
fi


###############################################################################
# VERIFY VAULT INTERMEDIATE CA CERTIFICATE
###############################################################################
heading_msg "VAULT INTERMEDIATE CA"
wget --quiet --output-document="$vault_intermediate_pem" $VAULT_ADDR/v1/$vault_intermediate_engine_name/ca/pem

title_msg "Check it is a CA certificate..."
result=$(openssl x509 -noout -text -in $vault_intermediate_pem | grep 'CA:TRUE')
if [ "$?" -eq 0 ]; then
...
echo Intermediate CA $(openssl x509 -noout -text -in $vault_intermediate_pem | grep 'Issuer:')

title_msg "Check that Intermediate was signed by root..."
openssl verify -CAfile $ext_intermediate_chain $vault_intermediate_pem

title_msg "Check that the Intermediate has a CRL..."
result=$(openssl x509 -noout -text -in $ext_intermediate_pem | grep -A 3 'X509v3 CRL Distribution Points' | grep URI)
if [ "$?" -eq 0 ]; then
...
result=$(openssl verify -crl_check -CAfile $ext_intermediate_crl_chain $vault_intermediate_pem)
if [ "$?" -eq 0 ]; then
...
issuers=$(vault list -format=json $vault_intermediate_engine_name/issuers | jq -r '.[]')
echo "$issuers" | while read line; do
    id=$line
    name=$(vault read -field issuer_name $vault_intermediate_engine_name/issuer/$id)
...
wget --quiet --output-document="issuing.pem" $base_url/$vault_issuing_engine_name/ca/pem
wget --quiet --output-document="issuing-chain.pem" $base_url/$vault_issuing_engine_name/ca_chain
...
# openssl verify -show_chain -CAfile $VAULT_ISSUING_CA $TEST_CERT

title_msg "Check the Vault Issuers"
issuers=$(vault list -format=json $vault_issuing_engine_name/issuers | jq -r '.[]')
echo "$issuers" | while read line; do
    id=$line
    name=$(vault read -field issuer_name $vault_issuing_engine_name/issuer/$id)
    echo "Found issuer [$id] with name [$name]"
done
...
vault write -format=json $vault_issuing_engine_name/issue/$vault_issuing_role \
...
vault write $vault_issuing_engine_name/revoke serial_number=$serial 1>/dev/null
